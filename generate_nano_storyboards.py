...
CANVAS:
- 9:16 vertical, approximately 768x1376.
- Premium commercial manhwa/webtoon art, crisp linework, realistic anatomy, controlled cell shading.
- Black title banner at top.
- Solid black gutters between panels.
- Reproduce reference storyboard structure: 8 visual shots arranged as 6 labeled beats: row 1 two panels, row 2 one full-width panel, row 3 two panels, row 4 two panels, row 5 one full-width final panel.
- Each beat label is bold white all-caps with black outline at lower edge of its visual beat.
- Timestamp labels total exactly 10 seconds: HUM? Use these six timestamps: 0s-1.5s, 1.5s-3s, 3s-4.5s, 4.5s-6s, 6s-8s, 8s-10s.
- Last label must include: END ON FREEZE FRAME.
...
CHARACTER REFERENCE SHEETS. Preserve exact faces and wardrobes from these references when visible:
{ref_paths}

DIRECTOR RULES:
- Show one coherent dramatic progression across all six beats, not six unrelated page thumbnails.
- Include visible camera composition in each panel: establishing, close-up, hero, reaction, macro insert, final wide.
- Use only story facts supported by Chapter 1 canonical analysis.
- Preserve character identity and wardrobe across panels.
- Text labels should be short and legible. Do not put dialogue paragraphs inside artwork.
- Final panel: explicit held pose, no new motion, designed as a freeze frame for Google Flow planning.
- No watermark, no random logos, no malformed text, no storyboard inside storyboard.
