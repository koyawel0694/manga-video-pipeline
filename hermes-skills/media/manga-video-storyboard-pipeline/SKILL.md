...
- Script fidelity: Every shot prompt and every `blockN_video_prompt.txt` continuous prompt must include the exact beat cue from the canonical ledger whenever it is provider-safe. Use `MANGA SCRIPT — Speaker: ... Exact line: "..."` for spoken/narrated text, `MANGA SOUND EFFECT (not spoken): ...` for SFX, and `MANGA SCRIPT: No spoken dialogue or voiceover for this beat. Do not invent dialogue.` for silent/transition beats. If an exact source cue would violate a provider safety policy, use an auditable `MANGA SCRIPT SAFETY REDACTION` with the speaker and source scene ID, keep the exact source unchanged in `chapter_script.json/.txt/.md`, and instruct the provider not to render, voice, caption, lip-sync, quote, or reenact it. Never invent, paraphrase, repeat, or move dialogue between beats.
...
9. **Canonical Script Is Mandatory**: Never send a storyboard or prompt package downstream unless `chapter_script.json` matches `chapter_analysis.json` exactly. SFX is sound design, not spoken dialogue. A transition beat may reuse a visual anchor only when it has an explicit no-dialogue cue. If a provider rejects a sensitive source cue, keep the canonical files unchanged and use an auditable `MANGA SCRIPT SAFETY REDACTION` in provider-facing prompts with the source scene ID.
